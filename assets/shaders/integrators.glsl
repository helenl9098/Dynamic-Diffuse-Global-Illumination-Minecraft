...
vec3 integrator_DDGI
    ...
    (Ray ray,    /* primary ray */
     ...
     ivec3 probe_counts, int side_length, vec3 field_origin)
...

{
    ...
    if (!intersect_scene(ray, mint, maxt, info)) return mix(white, blue, ray.direction.y);
...

{
    ...
    for (int i = 0; i < nrays; ++i)
    {
        ...
        if (intersect_scene(new_ray, mint, maxt, temp_info))
        {
            ...
            float lambert = clamp(
                dot(temp_info.normal, normalize(temp_info.pos - spheres[0].origin)), 0.0, 1.0);
        ...
        switch (info.mat.type)
        {
            case 0: /* direct Lambert */
                ...
                else
                {
                    /* Lambert's law, directional light */
                    float cos_light = max(0, dot(light_dir, normal));
                    return col + throughput * info.mat.base_color * light_intensity * cos_light;
                }
                break;
...

{
    ...
    for (int i = 0; i < nbounce; ++i)
    {
        /* intersected nothing -> background */
        if (!intersect_scene(ray, mint, maxt, info))
            return col;  // CHANGED: this makes background black. Statement below makes background
                         // white.
        // return col + throughput*mix(white, blue, ray.direction.y);
